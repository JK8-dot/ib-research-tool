...
def generate_pdf(ticker, fin_data, headlines, research_note):
    def clean(text):
    ...
    pdf.cell(0, 10, clean(f"{fin_data['name']} ({ticker})"), ln=True)
    pdf.set_font("Helvetica", "", 10)
    pdf.cell(0, 8, clean(f"Research Brief - {datetime.datetime.now().strftime('%B %d, %Y')}"), ln=True)
    pdf.ln(5)
    
    # Key metrics
    pdf.set_font("Helvetica", "B", 13)
    pdf.cell(0, 8, clean("Key Metrics"), ln=True)
    pdf.set_font("Helvetica", "", 10)
    pdf.cell(0, 7, clean(f"Price: ${fin_data['price']}   |   P/E: {fin_data['pe_ratio']}   |   Market Cap: {fin_data['market_cap']}"), ln=True)
    pdf.cell(0, 7, clean(f"6-Month Return: {fin_data['6mo_return']}%   |   Volatility: {fin_data['volatility']}   |   Sharpe: {fin_data['sharpe']}   |   Max Drawdown: {fin_data['max_drawdown']}%"), ln=True)
    pdf.ln(5)
    
    # Headlines
    pdf.set_font("Helvetica", "B", 13)
    pdf.cell(0, 8, clean("Recent Headlines"), ln=True)
    pdf.set_font("Helvetica", "", 10)
    for h in headlines:
        pdf.cell(0, 7, clean(f"- {h[:90]}"), ln=True)
    pdf.ln(5)
    
    # Research note
    pdf.set_font("Helvetica", "B", 13)
    pdf.cell(0, 8, clean("AI Research Note"), ln=True)
    pdf.set_font("Helvetica", "", 10)
    research_note = research_note.replace("—", "-").replace("–", "-").replace("\u2014", "-").replace("\u2013", "-")
    pdf.multi_cell(0, 7, clean(research_note))
    
    # Save
    filename = f"week-4/{ticker}_Research_{datetime.datetime.now().strftime('%Y-%m-%d')}.pdf"
    pdf.output(filename)
    return filename
...
for ticker in tickers:
    ...
    print(f"\n--- AI RESEARCH NOTE ---")
    print(research_note)
    filename = generate_pdf(ticker, fin_data, headlines, research_note)
    print(f"\n✅ PDF saved: {filename}")